...
//#ifdef TEST_SUITE
ZeParser._testSuite = function(callback){
	var inps = [
		...
		['/abc\//no_comment', 'one of the few cases where the double forward slash is actually not a comment'],
		['a: b; c;', 'label giving ASI a hard time, somehow'],
		['var x; function f(){ x; function g(){}}', 'function declaraton inside function body'],
		['if (x) { break }', 'apply asi after break, nowhere else'],
		['x.hasOwnProperty()', 'caused a problem with post processing (some obj had that property overwritten, hell broke lose)'],
		['(x) = 5', 'assigning to the result of the grouping operator is ok. and like this, it will actually work too'],
		['(x,x) = 5', 'on the parser level, this is fine. but the interpreter will kick your ass for it as the comma causes a fetch, screwing up your ref for assignment'],
		['switch(x){case 1:}', 'check for single clause and no body'],
		['while (x) { ++a	}', '++ and -- are exceptions in the parser eatStatement function'],
		['{break}', 'asi test for }'],
		['{continue}', 'asi test for }'],
		['{return}', 'asi test for }'],
		['{continue a}', 'asi test for }'],
		['{continue b}', 'asi test for }'],
		['{return c}', 'asi test for }']

		// to fail:
		// x() = y; (assignment to fnuction call)
		// +{f:}    (missing value)
		// }        (my parser ignores them on error for ASI)
		// try {}   (must have catch or finally)
		// switch(){ case f: )   (switch was looping endlessly due to special body case)
	];
...
ZeParser.testSuite = function(){
	var ok = 0;
	var fail = 0;
	var start = +new Date;
	ZeParser._testSuite(function(i, input, desc){
		var tok = new Tokenizer(input);
		var stack = [];

		try {
			new ZeParser(input, tok, stack).parse();
			++ok;
		} catch (e) {
			++fail;
		}
